...
module Navigation =
  ...
  let vddd_logo dispatch = 
    div [ Class "p-4 cursor-pointer flex-no-shrink flex items-center justify-center rounded-lg hover:bg-grey-light"  
    ...
    div [ Class "flex flex-col lg:flex-row items-start lg:items-stretch justify-end" ]
      [ render_entry "Sessions" (fun _ -> GoTo (Landingpage "sessions") |> dispatch)
        ...
        render_submenu "Social" 
          [ a [ Class "p-4 text-lg leading-tight cursor-pointer flex-no-shrink rounded-lg hover:bg-grey-light hover:text-blue-darker flex items-center justify-start lg:justify-center "
  ...
  let entry_mobile label onClick =
    div [ Class "w-full p-4 text-lg leading-tight cursor-pointer flex-no-shrink hover:bg-grey-light hover:text-blue-darker"
          OnClick (fun e -> e.stopPropagation() ; onClick() ) ]
      [ label |> ofString ]    

  let submenu_mobile label content =
    div [ Class "relative border-t border-grey-light w-full" ]
      [ div [ Class "absolute pin-t pin-r text-grey pt-2 pr-4 text-md" ]
          [ label |> ofString ]
        div [ ]
          content ]
  ...
  let submenu_desktop label content =
    button [ Class (sprintf "reveal-menu-content %s text-blue-dark focus:outline-none" clickable_item_style) ]
      [ label |> ofString
